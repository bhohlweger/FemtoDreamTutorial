#include "TROOT.h"
#include "TSystem.h"
AliAnalysisTaskSE* AddTaskFemtoDreamTutorial(
    bool isMC=false,
    TString CentEst="kInt7")
{
  ...
  TrackCuts1->SetPtRange(0.5, 4.05);
  ...
  TrackCuts1->SetPID(AliPID::kPion, 0.75);
  //We are looking for pions rejecting them would be obstructive.
  TrackCuts1->SetRejLowPtPionsTOF(false);
  ...
  TrackCuts2->SetPID(AliPID::kPion, 0.75);
  TrackCuts2->SetRejLowPtPionsTOF(false);
  ...
  AliAnalysisTaskFemtoTutorial *task=
      new AliAnalysisTaskFemtoTutorial("FemtoDreamDefault",isMC);
